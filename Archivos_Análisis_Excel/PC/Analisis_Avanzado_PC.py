...
print(f"Me parecia raro el resultado, hay un no adicto que ha perdido 10.000 euros")
media_no_adictos_sin_100000_euros = (dinero_perdido_no_adictos-10000) / (num_usuarios_no_adictos-1)  # Calcular el promedio
print(f"Valor medio de dinero perdido por usuario (No Adictos) quitando al de los 10000 euros: {media_no_adictos_sin_100000_euros:.2f} Euros por usuario")
...
# Definir las nuevas categorías de riesgo en orden y sus etiquetas correspondientes
categorias_riesgo = ["0.0", "1.0", "2.0", "3.0"]
etiquetas_riesgo = ["Sin Riesgo", "Riesgo Bajo", "Riesgo Medio", "Riesgo Alto"]

# Asegurar que la columna Riesgo_PC es string para evitar problemas con reindex
df_adictos["Riesgo_PC"] = df_adictos["Riesgo_PC"].astype(str)
df_no_adictos["Riesgo_PC"] = df_no_adictos["Riesgo_PC"].astype(str)

# Contar el número de usuarios en cada categoría de riesgo para adictos y no adictos
...
def calcular_puntos_proteccion(df):
    # Puntos por firewall
    df['puntos_firewall'] = df['Firewal_PC'].apply(
        lambda x: 2 if x == 'Tiene firewal activo' else 0)
    
    # Puntos por antivirus
    df['puntos_antivirus'] = df['Antivirus_PC'].apply(
        lambda x: 2 if (x != 'Ninguno' and pd.notna(x)) else 0
    )
    
    # Puntos por antiespías
    df['puntos_antiespias'] = df['Antiespías_PC'].apply(
        lambda x: 1 if x == 'Tiene' else 0
    )
    
    # Puntos por antispam
    df['puntos_antispam'] = df['Antispam_PC'].apply(
        lambda x: 1 if x == 'Tiene' else 0
    )
    
    # Puntos por antifraude
    df['puntos_antifraude'] = df['Antifraude_PC'].apply(
        lambda x: 1 if x == 'Tiene' else 0
    )
